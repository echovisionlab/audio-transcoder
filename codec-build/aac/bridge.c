...
EMSCRIPTEN_KEEPALIVE
int get_again_error_code(void) {
	return AVERROR(EAGAIN);
}

EMSCRIPTEN_KEEPALIVE
const char *get_error_description(int error_code) {
	static char description[AV_ERROR_MAX_STRING_SIZE];
	if (av_strerror(error_code, description, sizeof(description)) < 0)
...
int send_frame(EncoderContext *ctx, int64_t pts) {
	...
	int ret = av_frame_make_writable(ctx->frame);
	if (ret < 0) return ret;
...
EMSCRIPTEN_KEEPALIVE
int flush_encoder_start(EncoderContext *ctx) {
